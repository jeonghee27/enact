 ...
 * @private
 */
const VirtualListCoreSpottableNative = (BaseComponent) => (
	...
	class VirtualListCoreSpotlightManager extends Component {
		...
		__getIndexToScrollDisabled = (direction, currentIndex) => {
			const
		...
		scrollToNextItem = ({direction, focusedItem}) => {
			...
			if (Array.isArray(data) && data.some((item) => item.disabled)) {
				indexToScroll = this.__getIndexToScrollDisabled(direction, focusedIndex);
			} else {
...
export default VirtualListCoreSpottableNative;
export {VirtualListCoreSpottableNative};
