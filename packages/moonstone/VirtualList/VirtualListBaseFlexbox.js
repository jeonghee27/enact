...
import css from './VirtualListBaseFlexbox.less';
...
class VirtualListCore extends Component {
	...
	calculateScrollBounds (props) {
		...
		this.containerClass = css.list; /*(isPrimaryDirectionVertical) ? css.vertical : css.horizontal;*/
	}
	...
	applyStyleToExistingNode = (index, ...rest) => {
		const
			...
			node = this.containerRef.children[index];

		this.newCC[index] = this.cc[index];
	...
	applyStyleToNewNode = (index, ...rest) => {
		...
		this.newCC[key] = React.cloneElement(itemElement, {
	...
	positionItems ({updateFrom, updateTo}) {
		...
		this.containerRef.style.transform = 'translate3d(0px, ' + (-1 * (-primaryPosition % primary.itemSize)) + 'px, 0)';
		this.cc = this.newCC;
	...
	render () {
		...
		return (
			<div ref={this.initWrapperRef} className={mergedClasses} style={style}>
				<div {...rest} ref={this.initContainerRef} className={this.containerClass}>
					{this.cc}
				</div>
			</div>
		);
