...
import {I18nConsumerDecorator, I18nDecorator} from '../I18nDecorator';
...
describe('I18nDecorator', () => {
	...
	it('should update the current locale when updateLocale is called', function () {
		// eslint-disable-next-line enact/prop-types
		const Component = ({updateLocale}) => {
			const handleClick = () => updateLocale('ar-SA');
	...
	it('should update the rtl context parameter when RTL changes', function () {
		// eslint-disable-next-line enact/prop-types
		const Component = ({rtl, updateLocale}) => {
			const handleClick = () => updateLocale('ar-SA');
		...
		const Wrapped = I18nDecorator(
			I18nConsumerDecorator(
				{rtlProp: 'rtl', updateLocaleProp: 'updateLocale'},
				Component
			)
		);
		const subject = mount(<Wrapped />);
